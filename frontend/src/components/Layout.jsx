...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    const setupOfficerAuth = async () => {
      // Basic auto-login for demo purposes to avoid 401s on Officer routes
      try {
        const res = await api.post('/auth/login', { email: 'officer@gem.gov.in', password: 'password123' });
        localStorage.setItem('token', res.data.token);
        setAuthStatus("ready");
      } catch (err) {
        // If it fails, try to register the demo officer
        try {
          ...
          console.error("Failed to setup officer auth:", regErr);
          setAuthStatus("error");
        }
      }
    };
      ...
      <aside className="w-64 bg-navy-800 text-white flex flex-col shadow-xl z-10 sticky top-0 h-screen">
        <div className="p-6 border-b border-navy-900/50">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded bg-gold-600 flex items-center justify-center font-bold text-navy-900">G</div>
            <div>
